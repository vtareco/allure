	...
	if($currentPage < 3){
		for($i=1;$i<=5;$i++){
			if($list['pages'][$i] != null)
				$nav[] = $list['pages'][$i]['data'];
		}
		if($list['pages'][6] != null){
			$nav[] = preg_replace('#(<a.*?>).*?(</a>)#', '$1...$2', $list['pages'][6]['data']);
		}
	}else if($currentPage > $n-3){
		if($list['pages'][$n-5] != null){
			$nav[] = preg_replace('#(<a.*?>).*?(</a>)#', '$1...$2', $list['pages'][$n-5]['data']);
		}
		for($i=$n-4;$i<=$n;$i++){
			$nav[] = $list['pages'][$i]['data'];
		}
	}else{
		if($list['pages'][$currentPage-3] != null){
			$nav[] = preg_replace('#(<a.*?>).*?(</a>)#', '$1...$2', $list['pages'][$currentPage-3]['data']);
		}
...
function pagination_item_active(&$item)
{
	...
	if( $_REQUEST['option'] == 'com_villas' || $_REQUEST['option'] == 'com_realestatemanager'){
