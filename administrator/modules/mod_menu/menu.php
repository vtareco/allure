...
class JAdminCssMenu extends JObject
{
	...
	public function renderLevel($depth)
	{
		...
		//$component = $this->_current->getParent()->title;
		...
		if(is_array($my->groups))
			...
			//CMS @ 12-04-2015
			//Altera‹o dos nomes dos componentes quando apresentados no menu
			if($this->_current->title == 'RealEstateManager')
				$item = 'Manage Villas';
			else if($this->_current->title == 'jNews')
				$item = 'Newsletter';
			else
				$item = $this->_current->title;
			...
			if ($this->_current->link != null && $this->_current->target != null)
			{
				echo "<a" . $linkClass . " " . $dataToggle . " href=\"" . $this->_current->link . "\" target=\"" . $this->_current->target . "\" >"
					. $item . $dropdownCaret . "</a>";
					//$this->_current->title . $dropdownCaret . "</a>";
			}
			elseif ($this->_current->link != null && $this->_current->target == null)
			{
				echo "<a" . $linkClass . " " . $dataToggle . " href=\"" . $this->_current->link . "\">" . $item . $dropdownCaret . "</a>";//$this->_current->title . $dropdownCaret . "</a>";
			}
			elseif ($this->_current->title != null)
			{
				echo "<a" . $linkClass . " " . $dataToggle . ">" . $item . $dropdownCaret . "</a>";//. $this->_current->title . $dropdownCaret . "</a>";
			}
			else
			{
				echo "<span></span>";
