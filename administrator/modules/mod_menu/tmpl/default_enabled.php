...
$is_admin = in_array(7,$user->groups);
	...
	foreach ($components as &$component)
	{
		if (!empty($component->submenu))
		{
			...
			foreach ($component->submenu as $sub)
			{
				...
				if($component->text == 'jNews' && $sub->text == 'About' && !$is_admin){
					continue;
				}
				
				//CMS @ 10-04-2015 + VMT @ 29-12-2015
				//Controla o acesso aos submenus dos componentes de acordo com o utilizador logado
				if($is_admin ||
				   ($component->text == 'RealEstateManager' && ($sub->text == 'Villas' || $sub->text == 'Facilities' || $sub->text == 'Sale Manager')) ||
				   $component->text != 'RealEstateManager' )
				{
					if($sub->text == 'Sale Manager'){
						$sub->text = 'For Sale';
					}
		...
		else
		{
			...
			if($is_admin){
				...
				if($component->text != 'Unite Nivo Slider' && !stripos($component->text, 'helpdesk'))			
					$menu->addChild(new JMenuNode($component->text, $component->link, $component->img));
			}
		}
	}
 ...
 */
/**
 * Alteracao em 16-05-2015
 * @author CMS
 * So mostra o menu Help ao Administrador
 */
if ($showhelp == 1 && $is_admin)
{
