...
function com_install2() {
    ...
    if ($reallibr) {
		...
		`midweek` varchar(1250) default '',
		 ...
		 PRIMARY KEY  (`id`)		
            ...
            `mime_type` text
                ...
                require_once ($mosConfig_absolute_path . "/components/com_realestatemanager/lang/constant.php");
                if (isset($constMas)) {
                    foreach ($constMas as $item) {
                        $database->setQuery("INSERT INTO #__rem_const (const,sys_type) VALUES ('" . $item['const'] . "','" . $item['value_const'] . "')");
                        $database->query();
                    }
                }
                ...
                if (is_dir($component_path) && ($component_layouts = JFolder::files($component_path, '^[^_]*\.php$', false, true))) {
                    foreach ($component_layouts as $i => $file) {
                        ...
                        if ($file_name != 'constant') {
                            require_once ($mosConfig_absolute_path . "/components/com_realestatemanager/lang/$file_name.php");
                ...
                // load language
                $languagelocale = "";
                $database->setQuery("SELECT title, lang_code FROM #__rem_languages");
                ...
                $query = "UPDATE #__rem_version SET version = '" . substr($vers[0],0,3) . "', number = '" . $vers[1] . "' WHERE id = 2";
                $database->setQuery($query);
                $database->query();
                break;
            case 2.2:
        ...
        $query = "CREATE TABLE IF NOT EXISTS #__rem_houses (
              ...
              hdistrict varchar(50) NOT NULL default '',
              ...
              provider_class varchar(45) default '',
              ...
              hoa_dues varchar(45) default '',
              lot_size varchar(45) default '',
              model varchar(50) default '',
              property_taxes FLOAT(3,2) default '0.0',
              school varchar(45) default '',
              school_district varchar(45) default '',
              style varchar(45) default '',
              zoning varchar(45) default '',
        ...
        $query = "CREATE TABLE IF NOT EXISTS `#__rem_suggestion` (
                `id` int(11) unsigned NOT NULL auto_increment,
                `fk_userid` int(11),
                `user_name` VARCHAR(150),
                `user_email` VARCHAR(100),
        ...
        $database->query();
        echo $database->getErrorMsg();
        if (array_search($database->getPrefix() . "rem_suggestion", $database->getTableList(), true))
            echo "[Ok]<br />"; else
            exit;
    ...
    require_once ($mosConfig_absolute_path . "/components/com_realestatemanager/lang/constant.php");
    $borsch = count($constMas);
    $query = "INSERT INTO #__rem_const (const, sys_type) VALUES\r\n";
    foreach ($constMas as $k => $const) {
        if ($k != $borsch - 1)
            $query .= "('" . $const['const'] . "', '" . $const['value_const'] . "'),\r\n";
    }
    //print_r($constMas[$borsch-1]);
    $query .= "('" . $constMas[$borsch - 1]['const'] . "', '" . $constMas[$borsch - 1]['value_const'] . "');";
    $database->setQuery($query);
    $database->query();
    ...
    if (is_dir($component_path) && ($component_layouts = JFolder::files($component_path, '^[^_]*\.php$', false, true))) {
        foreach ($component_layouts as $i => $file) {
            ...
            if ($file_name != 'constant') {
                ...
                $database->setQuery("INSERT INTO #__rem_languages (lang_code,title) VALUES ('" . $LangLocal['lang_code'] . "'," . $database->quote($LangLocal['title']) . ")");
                $database->query();
                $idLang = $database->insertid();
                foreach ($constLang as $item) {
                    $database->setQuery("SELECT id FROM #__rem_const WHERE const = '" . $item['const'] . "'");
                    $idConst = $database->loadResult();
                    //	echo "<br>\n=============================== $$$ -" . $idConst . "- $$$ ====================================\n<br>\n";
                    if (isset($item['value_const']))
                        $value = str_replace("\"", "\\\"", $item['value_const']);
                    else
                        $value = str_replace("\"", "\\\"", $item[0]);
                    //	echo "<br> QUERY: '"."INSERT INTO #__rem_const_languages (fk_constid,fk_languagesid,value_const) VALUES (\"$idConst\", \"$idLang\", \"".$value."\")" ."'<br>";
                    if ((int) $idConst > 0) {
                        $database->setQuery("INSERT INTO #__rem_const_languages (fk_constid,fk_languagesid,value_const) VALUES (\"$idConst\", \"$idLang\", \"" . $value . "\")");
                        $database->query();
                    }
                }
            }
